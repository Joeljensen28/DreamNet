...
class WavenetDataset(torch.utils.data.Dataset):
    ...
    def calculate_length(self):
        ...
        for i in range(len(self.data.keys())):
            time_samples = self.data['arr_' + str(i)].shape[1]
            start_samples.append(start_samples[-1] + time_samples)
        print(f'Total samples: {start_samples[-1]}')
    ...
    def __getitem__(self, idx):
        ...
        print("one_hot: ", one_hot.shape, "example: ", example.shape)
